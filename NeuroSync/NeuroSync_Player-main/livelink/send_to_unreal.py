...
# Ensure Eyes blinking animation data in facial_data repeats after each len(default_animation_data) frames
def apply_blink_to_facial_data(facial_data: List, default_animation_data: List[List[float]]):
    ...
    for idx, frame in enumerate(facial_data):
        default_idx = idx % default_len # Modulus operation Loop through default animation data when idx exceeds its default_len
